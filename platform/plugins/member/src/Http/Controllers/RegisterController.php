...
class RegisterController extends Controller
{
    ...
    */
    ...
    {
        $dataCheck = Member::where('invitation_code', '=', $data['invitation_code'])->get();

        if (count($dataCheck) != 0) {
            $data['invitation_code_cv'] = $dataCheck[0]->invitation_code;
        } else {
            $data['invitation_code_cv'] = 0;
        }
        $rules = [
            'username' => 'required|unique:members,username',
            'phone' => 'required|min:10||max:11|unique:members,phone',
            'password'   => 'required|min:6',
            'confirm_password'   => 'required|min:6|same:password',
            'fund_password'   => 'required|min:6',
            'invitation_code'   => 'required|same:invitation_code_cv',
        ];

        $massages = [
            'invitation_code.same'   => 'Referral code does not exist',
        ];
        ...
        return Validator::make($data, $rules, $massages, [
